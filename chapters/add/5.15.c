...
struct Atom
{
    ...
    // Andersen Barostat
    double lastAndersenVelocity[3]; //  r/L*Pi/M
...
void Barostat_Andersen(double stress[6], double targetStress[3], int frequency, double timeStep)
{
    ...
    if (nStep == 0)
    {
        for (n = 0; n < atomNumber; n++)
        {
            for (i = 0; i < 3; i++)
            {
                atoms[n].lastAndersenVelocity[i] = 0;
            }
        }
    }
    if (count == 0)
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        for (n = 0; n < atomNumber; n++)
        {
            ...
            atoms[n].velocity[i] += AndersenVelocity - atoms[n].lastAndersenVelocity[i];
            atoms[n].lastAndersenVelocity[i] = AndersenVelocity;
...
int main()
{
    ...
    /* processing*/
    ConstructStdCrystal_BCC(3.11, 5);
    InitVelocity(300.0);
    Dynamics(5.0, 0.0001);
