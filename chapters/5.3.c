...
void VelocityMaxWell(double temperature)
{
    ...
    for (i = 0; i < atomNumber; i++)
    {
        mass = typeMasses[atoms[i].type - 1];
        for (d = 0; d < 3; d++)
        {
...
void ZeroMomentum()
{
    ...
    for (i = 0; i < atomNumber; i++)
    {
        for (d = 0; d < 3; d++)
        {
            momentum[d] += atoms[i].velocity[d] * typeMasses[atoms[i].type - 1];
        }
    }
    ...
    for (i = 0; i < atomNumber; i++)
    {
        for (d = 0; d < 3; d++)
        {
            atoms[i].velocity[d] -= momentum[d] / typeMasses[atoms[i].type - 1];
        }
    }
}
...
int main()
{
    ...
    printf("Total momentum (x,y,z): %f %f %f\n", momentum[0], momentum[1], momentum[2]);
