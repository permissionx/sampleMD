...
void Dump_lammpstrj(char fileName[20], int isNewFile, int nstep)
{
    ...
    for (n = 0; n < atomNumber; n++)
    {
        fprintf(fp, "%d %d %30.20f %30.20f %30.20f %30.20f %30.20f %30.20f %30.20f\n",
                atoms[n].id, atoms[n].type, atoms[n].r[0], atoms[n].r[1], atoms[n].r[2],
...
int main()
{
    ...
    strcpy(minimizeDumpFileName, "min.lammpstrj");

    /* processing & output*/
    ConstructStdCrystal_BCC(3.14, 10);
    // EdgeDislocation_100(3.14);
    //  double deleteBlock[3][2] = {{3.14*5-0.1, 3.14*5+0.1}, {3.14*5-0.1, 3.14*5+0.1}, {3.14*5-0.1, 3.14*5+0.1}};
    //  DeleteAtomsByBlockRegion(deleteBlock);
    double r[3] = {1, 1, 1};
    InsertAtom(r, 1);
    NeighborList(1);
